...
class PhotoVision {
    ...
    setupAIProviderListeners() {
        ...
        // Clear API settings button
        const clearApiBtn = document.getElementById('clearApiSettings');
        if (clearApiBtn) {
    ...
    async initializeProviderConfig() {
        try {
            ...
            if (data.success) {
                ...
                if (providerSelect && config.provider) {
                    ...
                    this.handleProviderChange(config.provider, false);
                }
                ...
                // Update API key field (don't show actual key for security)
                const apiKeyField = document.getElementById('apiKey');
                if (apiKeyField && config.hasApiKey) {
    ...
    handleProviderChange(provider, updateModels = true) {
        const modelSelect = document.getElementById('aiModel');
        const apiKeyField = document.getElementById('apiKey');
        
        if (!modelSelect || !apiKeyField) return;
        ...
        if (provider === 'openai') {
            ...
            apiKeyField.title = 'Enter your OpenAI API key';
        } else if (provider === 'claude') {
            apiKeyField.placeholder = 'sk-ant-...';
            apiKeyField.title = 'Enter your Anthropic API key';
        }

        // Enable/disable controls
        this.updateApiConfigControls();
    }
    ...
    updateApiConfigControls() {
        ...
        const hasApiKey = apiKeyField?.value?.trim();

        // Enable test button if we have provider, model, and API key
        if (testBtn) {
            testBtn.disabled = !(hasProvider && hasModel && hasApiKey);
        }

        // Enable save button if we have provider and API key
        if (saveBtn) {
            saveBtn.disabled = !(hasProvider && hasApiKey);
        }
    }
    ...
    async testCurrentApiConnection() {
        const providerSelect = document.getElementById('aiProvider');
        const modelSelect = document.getElementById('aiModel');
        const apiKeyField = document.getElementById('apiKey');
        const testBtn = document.getElementById('testApiConnection');

        if (!providerSelect || !modelSelect || !apiKeyField || !testBtn) return;
        ...
        if (!provider || !model || !apiKey) {
            this.addMessage('Please select provider, model, and enter API key before testing.', 'assistant');
            return;
        }
        ...
        try {
            this.addMessage(`Testing ${provider} connection with model ${model}...`, 'assistant');
            ...
            if (data.success) {
                this.addMessage(`✅ ${provider} connection successful! Response time: ${data.data.responseTime}ms`, 'assistant');
                
                // Update service status
                this.updateServiceStatus('ai-provider', 'connected', `${provider} connected`);
            } else {
                this.addMessage(`❌ ${provider} connection failed: ${data.error}`, 'assistant');
                this.updateServiceStatus('ai-provider', 'disconnected', 'Connection failed');
            }
        } catch (error) {
            console.error('Error testing API connection:', error);
            this.addMessage(`❌ Connection test failed: ${error.message}`, 'assistant');
            this.updateServiceStatus('ai-provider', 'disconnected', 'Connection error');
        } finally {
            // Re-enable button
    ...
    async saveApiConfiguration() {
        ...
        if (!providerSelect || !modelSelect || !apiKeyField || !saveBtn) return;
        ...
        if (!provider || !apiKey) {
            this.addMessage('Please select provider and enter API key before saving.', 'assistant');
            return;
        }
        ...
        try {
            this.addMessage(`Saving ${provider} configuration...`, 'assistant');
            ...
            if (data.success) {
                this.addMessage(`✅ AI configuration saved successfully! Using ${provider} with model ${model}`, 'assistant');
                
                // Update API key field to show masked value
                apiKeyField.value = '';
                apiKeyField.placeholder = '••••••••••••••••••••••••••••••••';
                
                // Update service status
                this.updateServiceStatus('aiProvider', 'connected', `${provider} configured`);
                
                // Enable chat functionality
                this.enableChat();
            } else {
                this.addMessage(`❌ Failed to save configuration: ${data.error}`, 'assistant');
            }
        } catch (error) {
            console.error('Error saving API configuration:', error);
            this.addMessage(`❌ Configuration save failed: ${error.message}`, 'assistant');
        } finally {
            // Re-enable button
    ...
    async clearApiSettings() {
        const confirmClear = confirm('Are you sure you want to clear all API settings? This will disconnect the AI provider.');
        ...
        try {
            ...
            if (data.success) {
                this.addMessage('✅ API settings cleared successfully.', 'assistant');
                ...
                if (providerSelect) providerSelect.value = 'claude';
                ...
                // Update service status
                this.updateServiceStatus('aiProvider', 'disconnected', 'No API Key');
                
                // Disable chat functionality
                this.disableChat();
            } else {
                this.addMessage(`❌ Failed to clear settings: ${data.error}`, 'assistant');
            }
        } catch (error) {
            console.error('Error clearing API settings:', error);
            this.addMessage(`❌ Failed to clear settings: ${error.message}`, 'assistant');
        } finally {
            if (clearBtn) {
