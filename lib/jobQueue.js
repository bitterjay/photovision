...
class JobQueue {
    constructor(rateLimiter = null) {
        ...
        this.config = {
            ...
            processingDelay: rateLimiter ? 500 : 1000, // Shorter delay when using rate limiter
            maxBatchSize: 1000 // Maximum jobs in a single batch
        };
    }
     ...
     * @param {Object} duplicateStatistics Optional duplicate detection statistics
     * @returns {String} Batch ID
     */
    addBatch(jobs, batchName = null, duplicateStatistics = null) {
        ...
        // Store duplicate detection statistics
        ...
        if (duplicateStatistics) {
            console.log(`[JobQueue] Duplicate statistics: ${duplicateStatistics.skippedImages} skipped, ${duplicateStatistics.newImages} new`);
        }
    ...
    async processJob(job, processors) {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                // Process the job with rate limiting if available
                let result;
                if (this.rateLimiter) {
                    result = await this.rateLimiter.execute(async () => {
                        return await processor(job.data, job);
                    });
                } else {
                    result = await processor(job.data, job);
                }
    ...
    getStatus() {
        ...
        return {
            ...
            duplicateStatistics: this.duplicateStatistics
        };
    }
