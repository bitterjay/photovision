...
class SmugMugClient {
    ...
    // Get album images optimized for preview (thumbnail data only)
    async getAlbumPreviewImages(accessToken, accessTokenSecret, albumUri) {
        try {
            console.log(`SmugMug: Getting preview images for album ${albumUri}...`);
            ...
            while (hasMore) {
                const response = await this.makeRequest(
                    ...
                    `${this.baseUrl}${albumUri}!images`,
                    {
                        _filter: 'ImageKey,FileName,Caption,ThumbnailUrl,LargeUrl,ArchivedUri,WebUri',
                ...
                const imagesData = response.data.Response.AlbumImage || [];
                const processedImages = imagesData.map(img => ({
                    imageKey: img.ImageKey,
                    filename: img.FileName || 'Untitled',
                    caption: img.Caption || '',
                    thumbnailUrl: img.ThumbnailUrl || img.Uris?.ImageSizes?.ThumbnailUrl || '',
                    largeUrl: img.LargeUrl || img.ArchivedUri || img.WebUri || '',
                    webUri: img.WebUri || ''
                }));
                ...
                // Check pagination
                const pages = response.data.Response.Pages;
                hasMore = pages && pages.NextPage;
                currentStart += pageSize;
                
                console.log(`SmugMug: Retrieved ${allImages.length} images so far...`);
            ...
            console.log(`SmugMug: Retrieved ALL ${allImages.length} preview images from album`);
            return {
                success: true,
            ...
            console.error('SmugMug get album preview images error:', error.message);
            return {
                success: false,
                error: error.message
            };
