...
class SmugMugClient {
    ...
    // Get user's albums with path information
    async getUserAlbums(accessToken, accessTokenSecret, userUri) {
        try {
            console.log('SmugMug: Getting user albums with path information...');
            
            const response = await this.makeRequest(
                'GET',
                `${this.baseUrl}${userUri}!albums`,
                {
                    // Request additional fields including UrlPath for hierarchy
            ...
            const rawAlbums = response.data.Response.Album || [];
            console.log(`SmugMug: Retrieved ${rawAlbums.length} albums`);
            
            // Process albums to add hierarchical path information
            const processedAlbums = this.processAlbumsWithHierarchy(rawAlbums);
