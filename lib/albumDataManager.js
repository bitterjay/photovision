...
class AlbumDataManager {
    ...
    // Update an image within its album
    async updateImageInAlbum(albumKey, imageKey, updates) {
        try {
            // Load the album
            const images = await this.loadAlbum(albumKey);
            ...
            if (imageIndex === -1) {
                throw new Error(`Image ${imageKey} not found in album ${albumKey}`);
            }
            ...
            // Update search index if keywords changed
            if (updates.keywords) {
                // Clear existing entries for this album first
                this.clearAlbumFromIndices(albumKey);
                // Re-index all images in the album
                await this.updateIndicesForAlbum(albumKey, images);
            }
            
            console.log(`Updated image ${imageKey} in album ${albumKey}`);
            return updatedImage;
        } catch (error) {
            console.error(`Error updating image in album: ${error.message}`);
            throw error;
        }
    }

    // Update search index for a single image
    updateImageInSearchIndex(image) {
        if (!image.smugmugImageKey || !image.albumKey) {
            return;
        }

        const albumKey = image.albumKey;
